	...
	for _, item := range configMapDatabase {
		err := Connection.Use(dbresolver.Register(dbresolver.Config{
			Replicas: []gorm.Dialector{mysql.Open(setDsn(
				os.Getenv(system.InterfaceToString(item["driver"])),
				os.Getenv(system.InterfaceToString(item["host"])),
				os.Getenv(system.InterfaceToString(item["port"])),
				os.Getenv(system.InterfaceToString(item["user"])),
				os.Getenv(system.InterfaceToString(item["password"])),
				os.Getenv(system.InterfaceToString(item["database_name"]))))},
		}, item["tables"]))
		if err != nil {
...
func (configDb configOtherDatabase) CreateOtherConnection() {
	if configDb.config != nil {
		for _, item := range configDb.config {
			err := Connection.Use(dbresolver.Register(dbresolver.Config{
				Replicas: []gorm.Dialector{mysql.Open(setDsn(
					system.InterfaceToString(item["driver"]),
					system.InterfaceToString(item["host"]),
					system.InterfaceToString(item["port"]),
					system.InterfaceToString(item["user"]),
					system.InterfaceToString(item["password"]),
					system.InterfaceToString(item["database_name"])))},
			}, item["tables"]))
			if err != nil {
				panic(fmt.Sprintf("Unable connection to database with error: %v", err))
			}
...
func mysqlDsn(host, port, user, pass, dbName string) string {
	return fmt.Sprintf("%s:%s@(%s:%s)/%s?charset=utf8mb4&parseTime=True&loc=",
		user, pass, host, port, dbName) + "Asia%2FJakarta"
}
